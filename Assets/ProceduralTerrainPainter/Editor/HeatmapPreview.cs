...
namespace sc.terrain.proceduralpainter
{
    public class HeatmapPreview
    {
        ...
        public static void CreateHeatmaps(Terrain[] terrains, int layerIndex, ref RenderTexture[] heatmaps)
        {
            ...
            for (var i = 0; i < heatmaps.Length; i++) heatmaps[i] = CreateHeatmap(terrains[i], layerIndex);
        }

        private static RenderTexture CreateHeatmap(Terrain terrain, int layerIndex)
        {
            var rt = new RenderTexture(terrain.terrainData.alphamapResolution, terrain.terrainData.alphamapResolution,
            ...
            var splatmap = terrain.terrainData.GetAlphamapTexture(splatIndex);
        ...
        public static void Draw(Terrain terrain, TerrainLayer layer, Texture alphaMap, bool contour, bool tilingPreview)
        {
            ...
            var pixelRect = new RectInt(0, 0, heightmapTexture.width, heightmapTexture.height);
            var pixelSize = new Vector2(terrain.terrainData.size.x / heightmapTexture.width,
                terrain.terrainData.size.z / heightmapTexture.height);
            var brushXform = TerrainPaintUtility.CalculateBrushTransform(terrain, new Vector2(0.5f, 0.5f),
                terrain.terrainData.size.x, 0.0f);
            ...
            //    +-+-+
            //
            var quadsX = pixelRect.width + 1;
            var quadsY = pixelRect.height + 1;
            var vertexCount = quadsX * quadsY * 2 * 3; // two triangles (2 * 3 vertices) per quad
            ...
            while
                (vertexCount >
                 kMaxFP32Int / 2) // in practice we want to stay well below 16 million verts, for perf sanity
            {
                quadsX = (quadsX + 1) / 2;
                quadsY = (quadsY + 1) / 2;
            ...
            var invWidth = 1.0f / heightmapTexture.width;
            var invHeight = 1.0f / heightmapTexture.height;
            ...
            var scaleX = pixelSize.x;
            var scaleY = terrain.terrainData.heightmapScale.y / kNormalizedHeightScale;
            var scaleZ = pixelSize.y;
            ...
            heatmapMat.SetVector("_ObjectPos_Offset",
                new Vector4(pixelRect.xMin * scaleX, 3f, pixelRect.yMin * scaleZ + pixelSize.y * 0.0f, 1.0f));

            // paint context origin in terrain space
            // (note this is the UV space origin and size, not the mesh origin & size)
            var pcOriginX = pixelRect.xMin * pixelSize.x;
            var pcOriginZ = pixelRect.yMin * pixelSize.y;
            var pcSizeX = pixelSize.x;
            var pcSizeZ = pixelSize.y;

            var scaleU = pcSizeX * brushXform.targetX;
            var scaleV = pcSizeZ * brushXform.targetY;
            var offset = brushXform.targetOrigin + pcOriginX * brushXform.targetX + pcOriginZ * brushXform.targetY;
            ...
            heatmapMat.SetVector("_LayerTiling",
                new Vector4(layer.diffuseTexture.width / terrain.terrainData.size.x / layer.tileSize.x,
                    layer.diffuseTexture.height / terrain.terrainData.size.z / layer.tileSize.y, 0, 0));
