...
namespace sc.terrain.proceduralpainter
{
    public class Utilities
    {
        ...
        public static Bounds RecalculateBounds(Terrain[] terrains)
        {
            ...
            foreach (var terrain in terrains)
            {
                if (terrain == null || terrain.terrainData == null)
                    throw new Exception(
                        "Failed to calculate total terrain bounds, one or more terrain objects are missing or broken");
        ...
        public static TerrainLayer[] SettingsToLayers(List<LayerSettings> layerSettings)
        {
            ...
            for (var i = layerSettings.Count - 1; i >= 0; i--) layerList.Add(layerSettings[i].layer);
