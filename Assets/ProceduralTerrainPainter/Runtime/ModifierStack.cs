...
namespace sc.terrain.proceduralpainter
{
    public class ModifierStack
    {
        ...
        public static void Configure(Terrain terrain, Bounds bounds, int resolution)
        {
            ...
            var invWidth = 1.0f / bounds.size.x;
            var invHeight = 1.0f / bounds.size.z;
        ...
        {
            for (var i = layerSettings.Count - 1; i >= 0; i--) ProcessSingleLayer(terrain, layerSettings[i]);
        }
        ...
        public static void ProcessSingleLayer(Terrain terrain, LayerSettings settings)
        {
            ...
            //Reverse order
            for (var i = settings.modifierStack.Count - 1; i >= 0; i--)
            {
            ...
            var scaledSplatmapSize = new Vector2(terrain.terrainData.size.x / m_resolution * m_resolution,
            ...
            var c = TerrainPaintUtility.BeginPaintTexture(terrain,
                new Rect(0, 0, scaledSplatmapSize.x, scaledSplatmapSize.y), settings.layer);
